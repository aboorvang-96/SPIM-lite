...
// ---------------------------------------------------------------------------
// Cycle helper — kept in step with app/hr/reports/attendance.tsx so both
// report screens default to the same 26→25 window.
// ---------------------------------------------------------------------------
...
export default function HrIncomeReport() {
  ...
  const scopeLabel = party.trim()
    ? `Party matches "${party.trim()}"`
    : 'All Employees / Parties';
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <Stack.Screen options={{ title: 'Income Reports' }} />
      ...
      <Card style={styles.card} mode="elevated" elevation={1}>
        <Card.Title
          title="Filters"
          subtitle="Party matches the same field the Income list searches (payment_by / title / source / description)"
          titleStyle={{ fontWeight: 'bold', color: theme.colors.secondary }}
        ...
        <Card.Content>
          <TextInput
            mode="outlined"
            label="Party / Payer (optional)"
            value={party}
            onChangeText={setParty}
            placeholder="Leave blank for all employees / parties"
            style={styles.input}
            dense
          />
